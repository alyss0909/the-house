---
name: The Numbers
offer_type: freebie
status: live
price_point:
sales_page_url: https://alyssacoleman.ca/formula
theme:
  - boh_systems
tags:
  - business
---

# The Numbers

**Part of:** [[business]]

## What it is

**"Know exactly how many people to grow your list by to hit your income goal."**

**Tier:** Freebie | **Theme:** Emails | **Price:** Free

A simple profit formula guide — no calculator needed — so you know if you're playing too small and exactly what list growth targets to aim for.

## Who it's for

Entrepreneurs who want to hit big monthly income goals but have no idea what the numbers behind that actually look like.

## Sales page copy

Full source copy: [[Library/Examples/sales-pages/the-numbers-sales-page|The Numbers Sales Page]]

Source URL: <https://alyssacoleman.ca/formula>


## Notes

## Related notes

## Wiki

- Sales page: [[Library/Examples/sales-pages/the-numbers-sales-page|The Numbers Sales Page]]
- Facts (price/status/chat word): [[Studio/Hermes/BUSINESS]] — canonical
- BOH teaching module: [[Library/Programs/back-of-house/0. Welcome/0.3 Cozy Business Plan|0.3 Cozy Business Plan]] (Cozy Biz Plan math, W3 Mon slot per MAP §3/§4)
- Real posts that pitched it: no matching carousel found in `Library/Examples/Carousels/` (GAP per MAP §3 W3 Mon)
- Funnel neighbors: none documented in [[Studio/Hermes/BUSINESS]] funnel chains

