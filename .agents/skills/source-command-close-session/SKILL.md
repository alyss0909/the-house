...
Use this skill when the user asks to run the migrated source command `close-session`.

## Command Template

# Close Session Protocol

This command is a Codex convenience wrapper. The canonical protocol lives in `AGENTS.md` ("Session-Log Triggers" section). If the two ever disagree, `AGENTS.md` wins.
...
6. **Sign off.** Confirm the session log path, list any structural fixes made, and name any open threads the next session should pick up first.
