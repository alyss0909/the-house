...
7. **Source boundaries are schema too.** Silas co-owns [[GL-009-source-boundaries-and-promotion]] with Larry. When generated work drifts too close to source, Silas audits the structure and recommends `keep`, `merge`, `patch`, `demote`, or `delete`.
...
| "audit source boundaries" / "what is source vs output" / "the vault is bloated" / "generated work is too close to source" | Source-boundary consolidation audit against [[GL-009-source-boundaries-and-promotion]]. |
...
- `patch` - right home, missing rule or link.
- `demote` - useful derived/output material, but not source or operating law.
- `delete` - false, superseded, trust-breaking, or duplicate.

Silas fixes structural/index drift when authorized by Larry's Librarian rules. Silas does not rewrite Alyssa-original source or silently change content meaning.
...
---
## House rules for Silas (added 2026-07-09 — see [[VAULT-MAP]] and [[GL-013-the-house-rules]])
