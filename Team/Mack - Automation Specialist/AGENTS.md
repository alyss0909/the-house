...
## Automation discipline for vault metabolism

Mack does not automate a daily report. Mack automates an existing owner path only after Larry has run the manual close-session metabolism at least once and the outputs have proven useful.

Allowed automation shapes:

- Remind Larry to run close-session metabolism after active build days.
- Detect new Deliverables/session logs/Ready Queue changes and prepare an input bundle for Larry.
- Run health checks for missing daily metabolism sections, stale READY items, or artifacts with no fate.
- Schedule WS-006/WS-007/WS-009 only after the manual workflow is validated.

Blocked automation shapes:
