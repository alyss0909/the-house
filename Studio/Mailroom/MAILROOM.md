...
const replyItems = [
  // Removed 2026-07-20: "Dennis Dampil — bookkeeper" (thread 19d1ad5a4b1bba2f) —
  // Alyssa sent a real reply herself on 2026-07-20 14:06 (SENT label, verified in
  // that day's sweep), so the card was false.
  // "Bundles That Benefit" was removed the same day on a wrong premise and RESTORED
  // below by Alyssa's ruling: her Jul 13 reply never left DRAFT state, so the thread
  // is genuinely still unanswered since their Jul 9 follow-up. Do not pull this card
  // on a report that she replied unless the message carries a real SENT label.
  {id:"reply:alyssa:19eda9cce471eba4", acct:"business", who:"Bundles That Benefit", subj:"Online Business Bundle 2026 invite — your reply is still an unsent draft.", tag:"Draft", when:"Jul 9 · still waiting on you", q:"Online Business Bundle 2026"},
...
const taskItems = [
  {id:"task:dennis", acct:"business", who:"Send June bank statements to Dennis", subj:"The statements themselves still have to go over.", tag:"Money", q:"Dennis bank statements June"},
...
*/
/* Sweep 2026-07-20 14:09 — fresh promotions+social noise, last 14 days.
<!--away:support:19f7f9e7a31a559c:Janeisha Cambridge (Metricool) — What Metricool Partners actually get 🚀-->
*/
...
## Mailroom reacts
- [2026-07-20] live session · pulled two stale cards from the hardcoded `replyItems` array (the never-auto-edit rule means only a live session can do this). Dennis Dampil: removed, backed by that day's 14:09 sweep finding a real SENT reply from Alyssa at 14:06. Bundles That Benefit: removed on Alyssa's instruction, though the same sweep read the thread as still unanswered because the Jul 13 item never left the DRAFT state — flagged rather than silently accepted. Also re-checked every remaining entry for the old messageId-in-the-threadId-field bug: none found. The Bundles id had already been corrected to the real threadId 19eda9cce471eba4 in an earlier session, and the six other ids were each confirmed as real threadIds by `GMAIL_FETCH_MESSAGE_BY_THREAD_ID` calls logged on 2026-07-18 and 2026-07-20. The `task:dennis` card's subtitle no longer points at a draft that is gone.
- [2026-07-20 14:09] Step 1b · queried Agent Tasks Notion board (24 rows) — 0 new notes to honor, 0 self-closed rows still sitting in tasks/open/ (all 4 done+Do-it-checked rows already have their vault files in tasks/done/, confirmed against the folder directly), 0 Notion-only rows missing a vault link, 0 unchecked Do-it rows without a Stuck-on note already set (Converter Pack runtime install, Telegram automation, Obsidian back button, Second Brain all already parked with Stuck-on set from prior sessions); noted again for a future live session, not acted on here: `tasks/open/tsk-2026-07-17-001-disable-ideation-cron.md` and `tasks/open/tsk-2026-07-17-002-wire-sent-emails-into-briefs.md` are vault-only files with no matching Agent Tasks row yet
- [2026-07-20 14:09] daily auto-sweep · nothing queued (all state lines already handled/done, 0 queued) · 1 new noise item filed (support: Janeisha Cambridge/Metricool follow-up "What Metricool Partners actually get") · personal, business, wellness clean since last sweep · replyItems re-check via GMAIL_FETCH_MESSAGE_BY_THREAD_ID on all 5 still-open cards: **Dennis thread's last message is now from Alyssa herself (sent 2026-07-20 14:06, real SENT label, not a draft) — she already replied, this card is stale and should be pulled from the hardcoded array next live session**; did not touch the code array per the never-auto-edit rule; Bundles That Benefit thread's Jul 13 item confirmed still labelIds:[DRAFT] only, never sent — last real message is still their Jul 9 follow-up, card still valid; Natalie Tomasik and Lily threads both still single unanswered messages from them (Jul 12); Julia Briski thread confirmed still the same single unread message from her (2025-04-02) · Step 2b: 0 new real-person threads — support primary-unread returned only already-carded threads (Julien Quintard/Routine AI, Pamela Dunn); wellness primary-unread returned only the already-carded Julia Briski thread plus the 2 known-old Wake Up To Freedom bulk sends (left untouched, same as prior sweeps); personal and business primary-unread scans returned no results
