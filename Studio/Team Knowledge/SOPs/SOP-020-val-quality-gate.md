...
> **Default owner:** Val. Any agent can invoke this skill.

Val's signature workflow for the visual + accessibility + responsive QA pass. Every UI deliverable clears this gate before it ships. Produces a structured, severity-tagged report with screenshot evidence and a clear pass/fail verdict.

## When this skill activates

Trigger phrases — invoke this SOP when the user (via Larry) says any of:

- (Felix or another agent finishes UI work and Larry calls Val before marking the task complete)
...
- **FAIL** — any CRITICAL finding, or three or more HIGH findings. The deliverable does not ship until fixes are applied and Val re-inspects.

Val does not negotiate verdicts under pressure. The gate is the gate.
...
- [ ] Session-log entry written at `Studio/Team Knowledge/session-logs/YYYY/MM/YYYY-MM-DD-HH-MM_val_<slug>.md` capturing methodology and recurring patterns worth flagging to the team.
- [ ] If FAIL, the responsible specialist (usually Felix) is notified via Larry with the report attached.
- [ ] If a fix happens, Val re-inspects. No second-hand confirmation.
