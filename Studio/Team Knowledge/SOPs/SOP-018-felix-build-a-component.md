...
Felix's signature workflow for building a UI component end-to-end with design-system fidelity, type safety, accessibility, and a clean handoff to Val's quality gate.
...
TypeScript compiling is necessary but not sufficient. Before handing off to Val:
...
If anything looks wrong, fix it now. Val's gate is faster when you've already cleared the obvious self-checks.
...
The build isn't done until Val signs off. Larry routes the deliverable to Val; Val runs [[SOP-020-val-quality-gate]] and returns a pass/fail. If Val flags issues, fix them, then resubmit. Don't argue the gate.
