...
- **References:** [[SOP-008-rebuild-task-index]], [[SOP-006-close-task]], [[SOP-010-read-own-journal]]
...
6. **Rebuild the index.** Run [[SOP-008-rebuild-task-index]].
...
3. **If the blocker is itself a task** (someone else's open work), create that task via [[SOP-004-create-task]] and reference it in `blocked_by`:
   ```yaml
...
Mack runs [[SOP-007-list-open-tasks]] and sees:

```
[priority 1] tsk-2026-05-09-001-mux-webhook-401 — assignee: mack
```

Pre-flight: opens the file, reads `linked_sops: [SOP-005-claim-task]`, `linked_session_logs: [2026-05-09-22-30_larry_video-launch-coordination]`, `linked_journal_entries: []`. Walks the session log to recover the launch context.
...
Runs [[SOP-008-rebuild-task-index]]. Reports back: `Claimed [[tsk-2026-05-09-001-mux-webhook-401]], digging in.`
