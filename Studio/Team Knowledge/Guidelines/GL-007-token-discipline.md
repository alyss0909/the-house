...
| Analysis files | 600 lines | `writing-style.md` at 716 lines is the historical ceiling — do not exceed it in new files |
| Framework files | 400 lines | |
| Routing guides | 300 lines | COMPASS is the model; `copy-task-routing.md` at 545 lines is the detailed reference ceiling |
| Templates | 200 lines | |
...
Larry reads `CLAUDE.md` (brief pointer) + `AGENTS.md` (~292 lines) on every session. This is the unavoidable floor.

To keep the floor from growing:
- `AGENTS.md` should not grow beyond 300 lines without a structural review
...
- **If the new content genuinely needs its own file:** create it, update the relevant INDEX.md, and update `COMPASS.md` if it's a commonly-used task type
...
- **Never add content to `COMPASS.md` beyond 120 lines.** If the compass is growing, the routing logic belongs in `copy-task-routing.md`, not the compass itself
