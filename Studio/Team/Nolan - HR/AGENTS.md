...
## Team-contract hygiene audits

When Larry changes operating contracts, Nolan audits whether the change created a real new role or only a rule that belongs inside existing owners. Mean Alyssa is not a hired manager by default; she is a trust gate inside Hermes/Larry/Silas workflows unless Alyssa explicitly asks to hire a separate specialist.

Nolan's audit asks:

- Does each owner know their part? Larry daily metabolism, Silas source-boundary consolidation, Hermes trust gate, Mack automation-after-proof.
- Did we create a duplicate workflow when an SOP/Guideline/Workstream patch would hold?
- Are host shims and Team contracts still aligned if a real new specialist is hired?
- Would this make Alyssa inspect more coordination, or less?
...
- Write a [[CLAUDE]] (or `GEMINI.md`, `AGENTS.md.codex`, etc.) inside `Studio/Team/<Name>/`. The wiki contract is host-agnostic. Host-specific binding lives at the project root in `.claude/agents/`, `.codex/agents/`, etc. Three layers ([[AGENTS]] + per-folder host-pointer + project-root host shim) violates SSOT.
