...
- **Tag audit.** Larry checks tags against [[GL-004-tag-taxonomy]] at every session close and at least once per active day if work spans a day without a formal close. The audit flags tags not listed in GL-004, prohibited structural tags in Second Brain content, journal-legacy tags outside Journal, low-use tags under 3 uses, inline body hashtags accidentally becoming Obsidian tags, and duplicate/plural/abbreviation variants. Larry fixes clear structural drift and asks Alyssa before changing content meaning.
- **Inbox \_Processed/ clear.** If `Notebook/Inbox/_Processed/` contains files, Larry deletes them via PowerShell (Larry in main thread can delete; subagents cannot). This is a session-close routine, not a user action.
- **Daily knowledge metabolism.** On active build days, Larry connects every chat, agent output, Deliverables file, task/open loop, Ready Queue move, Hermes correction, and unresolved thread back to the day's larger objective. Every artifact gets a fate: `promoted`, `referenced`, `dispatched`, `archived`, `deleted`, or `needs-Alyssa`. "Interesting" is not a fate.
...
## Daily knowledge metabolism
**Today's larger objective:** ...
**What connected:** ...
**What changed in source maps / tasks / Hermes / Ready Queue:** ...
**What was superseded, demoted, archived, or deleted:** ...
**What still needs Alyssa, Pax, Mack, or source access:** ...
...
| "what tags should I use", "is this tag approved", "update the tag list", "audit tags against approved tags" | Larry owns tag governance via [[GL-004-tag-taxonomy]]; Silas supports with frontmatter/tag parsing and batch cleanup when routed |
...
This is the path from an explicit "remember this" moment to a specialist's journal. The session log captures chronology; the journal entry makes the insight retrievable at the right moment.
...
---
## House rules for Larry (added 2026-07-09 — see [[HOUSE-MAP]] and [[GL-013-the-house-rules]])
