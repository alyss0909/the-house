# Soft Sundays

Her real examples, in full, live here:

- [[Library/Examples/soft-sundays/why-i-burned-down-my-business]]
- [[Library/Examples/soft-sundays/i-failed-formula-for-growth]]
- [[Library/Examples/soft-sundays/i-fixed-your-email-list]]
- [[Library/Examples/soft-sundays/if-lorelai-had-an-online-biz]]
- [[Library/Examples/soft-sundays/marketing-predictions-2026]]
- [[Library/Examples/soft-sundays/me-i-need-ideas-i-already-had]]
- [[Library/Examples/soft-sundays/ur-not-new-march-plan]]
- [[Library/Examples/soft-sundays/your-list-called-theyre-hungry]]

Write toward these. Read at least two in full, including [[Library/Examples/soft-sundays/why-i-burned-down-my-business]], before drafting a new Soft Sunday.

## What makes hers hers

Pulled only from patterns actually visible in the examples above.

- **Fixed section structure, repeated every week.** The Pep Talk, The House Report, Time Luxuries, On The Calendar, The Check In. Each section header carries its own italic tagline in parentheses, like "the weekly pep talk you didn't know you needed (from your business big sis)."
- **The House Report teaches in numbered "reasons," each a full scene with its own problem and pivot, not a thin bullet.** "Reason nO1. I wanted my business to be more creative" runs several paragraphs with its own confession, mistake, and fix before Reason nO2 starts.
- **She quotes her own past self.** "It was around March of 2024 when I felt the slow dread start to sink in" places a specific month and a bodily sensation together.
- **The Check In section ends on an open question for the reader to answer, not a CTA.** "my week would be a success if the only thing I did was ________________." The offer link comes after, in a Psst.
- **Long sentences are the default register; a short staccato line appears only once, at the emotional high point.** "Gone." on its own line after a full paragraph of setup in [[Library/Examples/simmers/when-delta-stole-my-business-idea]] is the same device this format shares with Simmers.
- **Time Luxuries section is reflective and unhurried, closer to a journal entry than marketing copy**, e.g. "2024 felt like tearing down a house I wasn't sure I wanted to live in anymore."

## Hard guard: bold density

**3-6 bolds per 200 words in The House Report**, and if only the bolds are read, the argument still has to hold. Products appear only after the teaching earns them, never cold. Full detail in [[Hermes/craft/emails-soft-sunday]]. (Note: some long-form origin-story emails run well above this ceiling in the corpus — that tension is still open and unconfirmed by Alyssa, see the audit's stale-items list; don't treat the higher number as a rule.)
