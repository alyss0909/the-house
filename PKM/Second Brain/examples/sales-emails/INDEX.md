---
name: Sales Email Examples - Index
type: index
last_updated: 2026-07-04
tags: [email, promo, copy]
---

# Sales Email Examples - Index

> Full-text sales emails written by Alyssa. These are actual promotional emails sent to her list - push-to-buy copy with real CTAs, real offers, real urgency language. Read before writing any promo email.

**Before using these examples, read:**
- [[analysis/writing-style|analysis/writing-style.md]] - voice patterns, rhythm, sentence structure, what makes Alyssa sound like Alyssa
- [[analysis/subject-line|analysis/subject-line.md]] - subject line formulas and what has performed

---

## Sales Emails

| File | Subject line | Topic | Notes |
|---|---|---|---|
| [boh-sales-email-pinterest-plan.md](boh-sales-email-pinterest-plan.md) | (promotional - subject line not preserved in export) | Back of House program - Pinterest + email system for booking clients | 4-step Pinterest-to-email plan, BOH department overview, coupon ORDERUP, deadline urgency |
| [slowly-viral-webinar-invite.md](slowly-viral-webinar-invite.md) | [INVITE inside] Slowly Viral free pinterest class inside | Launch invite for Slowly Viral free class (Feb 26) - reframes virality, pitches slow compounding Pinterest strategy | Willy Wonka golden ticket metaphor; research cite (2025 Scientific Reports); BOH soft open teaser |
| [slow-marketing-plan-webinar-invite.md](slow-marketing-plan-webinar-invite.md) | Your 2026 business plan inside | Launch invite for Slow Marketing Plan free class (Oct 22) - antisocial-media system, Pinterest + email, 19.4-hour content stat | BOH early access waitlist CTA; segment opt-out footer preserved |
| [silent-killer-of-good-work.md](silent-killer-of-good-work.md) | The silent killer of GOOD work. (pls avoid!) | Creatives-with-high-standards positioning - "make things" contrast, compromising quality just to keep up, BOH as the system that gives creative time back | 4-step framework (Build -> Automate -> Grow -> Sell); CTA reply "Ghost your Feed"; enrollment segment opt-out preserved |
| [email-list-resuscitator-oops-email.md](email-list-resuscitator-oops-email.md) | Oops / last chance email | Email List Resuscitator | 70% off last-chance oops email; source example for `email-oops.md` |

---

## How to use these

**For voice calibration:** Read the opening hook and the TLDR section. Notice how she frames the product as a system you install once, not a course you take.

**For structure reference:** Alyssa's sales emails lead with a plan or framework first (value-up front), then reveal the offer as the vehicle for implementing it. The CTA appears multiple times, always with a clear reason to click now.

**Rule from examples/INDEX.md:** Always read at least one real example before writing. Never start cold.
