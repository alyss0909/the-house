...
|---|---|---|---|
| 1 | BOH price + canonical source | **PASS** | Old map named `PKM/My Life/Offers/` for Offers directly. `back-of-house.md` → price 3000, chat word/price canonical at `Hermes/BUSINESS.md`. One hop. |
...
| 9 | Chat word for BOH + source | **PASS** | Same path as Q1 — `Hermes/BUSINESS.md` named as canonical inside the offer file. |
...
|---|---|---|---|
| 1 | BOH price + canonical source | **PASS** | $3,000. `Hermes/BUSINESS.md` canonical; `Library/Offers/back-of-house/MAP.md` correctly defers to it. 4 hops. |
| 2 | Real carousel that pitched Pin Planner | **PASS** | `Library/Examples/Carousels/2026-01-29-smart-founders-going-low-maintenance` — verbatim caption confirmed in `Library/Examples/Carousels/captions.md`. 4 hops. |
| 3 | What Alyssa said about slow marketing | **PASS** | "Slow marketing got me more clients than daily posting ever did." — `Notebook/Journal/2026/04/2026-04-30-hooks.md`. |
| 4 | Where new AI voice-analysis belongs | **PASS** | `Studio/Analysis/` — explicit rule found in `Library/MAP.md`'s "What is NOT here" section, confirmed against `Studio/MAP.md`. 3 hops. |
| 5 | Where raw Telegram drops land | **PASS** | `Notebook/Inbox/`, named explicitly (with Telegram called out) in `Notebook/MAP.md`. 2 hops. |
| 6 | File that owns the weekly pitch state | **PASS** | `Studio/Content/2026-W30-pitch.md` (newest), per the explicit single-state-machine rule in `Studio/Content/dashboard/PIPELINE-CONTRACT.md`. 4 hops. |
| 7 | Is `voice-profile.md` trusted source or opinion — how do you know | **PASS** | Team opinion — confirmed by rule text in `Library/MAP.md` AND the file's own frontmatter (`status: AI-mined draft — awaiting Alyssa review`). Two independent confirmations. |
...
| 9 | Chat word for BOH + source | **PASS** | "Yes Chef" — `Hermes/BUSINESS.md` canonical. |
...
**What it did, cold:**
1. Read `HOUSE-MAP.md` (the mandatory root map) — correctly identified Notebook as read-only source, Studio as the workshop.
2. Read `Notebook/MAP.md` — confirmed journal location, confirmed it's read-only.
3. Read `Studio/MAP.md` — correctly distinguished the **Signals** desk ("AI-detected patterns and theses from the journal") from the **Analysis** desk (voice/copy pattern work) and ruled Analysis out for this task without being told the difference — it worked that out from the desk descriptions alone.
4. Read `Studio/Signals/signals-index.md`, an existing garden-note template, and one real garden note to learn the vault's actual filing convention before writing anything.
5. Read `Studio/Team Knowledge/SOPs/SOP-013-semantic-sweep.md` — found this is the documented procedure for exactly this task, and explicitly flagged in its own output that a 3-entry ad hoc read is a lighter version of the "light sweep" rather than a formal scheduled sweep — an honest scope distinction it made on its own.
...
**What it produced:** a garden note at `Studio/Signals/Developing Ideas/system-already-beats-external-tools.md` (the recurring theme: three tool-evaluation entries all conclude "we already built this, better") and a matching entry in `signals-index.md` — both following the vault's existing conventions exactly, wikilinked to the three source journal entries, the signals index, SOP-013, and one adjacent existing signal so nothing is an orphan.
