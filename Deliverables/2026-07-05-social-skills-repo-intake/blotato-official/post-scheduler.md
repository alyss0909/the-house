> For the complete documentation index, see [llms.txt](https://help.blotato.com/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://help.blotato.com/claude-skills/claude-skills/post-scheduler.md).

# post-scheduler

Schedules a finished social post to one or more platforms via the Blotato API. Handles single-platform and multi-platform scheduling, fetches connected accounts, runs a final pre-publish check, and returns scheduled time and post IDs. Falls back to saving the post as a copy-paste file if Blotato isn't configured.

## Install

1. Create a folder: `~/.claude/skills/post-scheduler/`
...
For Blotato API access, see [Claude Code MCP setup](/api/claude-code.md). Without Blotato, post-scheduler falls back to saving the post as a copy-paste text file.
...
* [post-writer](/claude-skills/claude-skills/post-writer.md): produces the graded post post-scheduler ships
* [post-grader](/claude-skills/claude-skills/post-grader.md): runs before post-scheduler to confirm quality
* [content-coach](/claude-skills/claude-skills/content-coach.md): orchestrator calling post-scheduler last
...
```
GET https://help.blotato.com/claude-skills/claude-skills/post-scheduler.md?ask=<question>&goal=<endgoal>
```
