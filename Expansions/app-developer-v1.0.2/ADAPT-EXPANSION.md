...
- `requires_agents: [Larry, Nolan]` — both pre-hires must be present (they are by default in v1.7.0+). This is a pure `agent_pack` with no connectors, so Mack is not required and is not listed.

## What WS-003 does for this Expansion

### Step 1 — Larry: manifest preflight

Larry reads `expansion.yaml`. Confirms `requires_agents` (Larry, Nolan) are present in the user's team. Shows the user a preview: 3 agents, 3 SOPs, 0 env vars, 0 runtimes, 0 guidelines, 0 workstreams. Two informational `post_install_steps` to relay.
...
If all four pass and the manifest hash matches the pinned `app-developer@<version>` line → GREEN. A hash mismatch is the only path to YELLOW (pin-then-GREEN on user consent).
