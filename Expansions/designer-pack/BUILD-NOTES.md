...
Patch release. **Removed the scaffold-version gate** so the pack installs regardless of the host scaffold version. Specifically:

- Deleted `requires_scaffold_version: ">=1.7.0 <3.0.0"` from `expansion.yaml`. The `<3.0.0` upper bound was a live ceiling that blocked installs on 3.x scaffolds; rather than re-pin the range again, the gate is dropped entirely. Scaffold-presence, `requires_agents` (agent-presence), and the Vex security gate are all unchanged and still enforced.
- Removed the `requires_scaffold_version` line from the "Manifest contract" section of `ADAPT-EXPANSION.md`, and dropped the "Confirms `requires_scaffold_version` is satisfied against the current scaffold" instruction from WS-003 Step 1 (Larry: manifest preflight). Larry still confirms `requires_agents` presence.
- Bumped `version` 1.1.0 → 1.1.1 in `expansion.yaml` and the `ADAPT-EXPANSION.md` header. No agent, SOP, guideline, or other content changed.

Historical `requires_scaffold_version` references below (v1.0.0 / v1.1.0 sections) are left intact as build provenance.
