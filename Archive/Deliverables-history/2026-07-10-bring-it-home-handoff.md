...
- **Prose:** SOPs/Workstreams/Guidelines cross-references (most use bare `[[SOP-xxx]]` basename links which survive; only full-path references need edits — grep, don't guess); [[Studio/Team/agent-index]] self-references; VAULT-MAP transition table + Studio/MAP desk rows.
...
- **Hermes rewrite (CRITICAL, silent-failure risk):** [[Hermes/SOURCES]] + `MAP.md` tier-1 loads repointed to Library paths; `.claude/agents/hermes.md`, `cass.md`, `suki.md` hardcoded Second Brain paths; GL-010's embedded prompt template. Hermes/BUSINESS.md demotes to a values-free pointer table per GL-013 ("Hermes is judgment, not a brain") — each offer's facts live in its Library MAP; BUSINESS.md becomes offer-name → wikilink rows only.
- Remaining PKM: [[Library/me]] (flag for Alyssa's review-then-promote), [[Studio/Analysis/taste-log]]/[[Studio/Analysis/yes-pattern]] → Studio/Analysis; `.user.yaml` → stays at a path AGENTS.md's Personalization section is updated to match (root `.user.yaml` is cleanest); `Documents/` (empty scaffold) → decision for Alyssa; then retire `PKM/` with a MOVED stub.
...
Write both results into [[Deliverables/2026-07-10-validation-results]] with real wikilinks.
