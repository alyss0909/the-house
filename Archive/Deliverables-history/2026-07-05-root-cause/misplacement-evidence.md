...
- **CTC** = "Content That Converts," a separate paid product. It surfaces repeatedly in Journal/offers content ([[Notebook/Journal/2024/04/2024-04-06-notion-ideas]], [[Notebook/Journal/2025/02/2025-02-27-current-big-ideas]]) as an existing course Alyssa wants repurposed/turned into an email course — but it has **no offer file in `PKM/My Life/Offers/`** and no Second Brain content at all. It is referenced only as a future-import target in [[Studio/Analysis/BUILD-PLAN]] ("CTC content index — not started") and open-thread lists in three separate session logs (2026-06-03, 2026-06-04) and the W23 weekly wrap. It was never sent, never imported, never duplicated elsewhere.
- **YOE** = "Year of Emails," a real, long-running product (has its own offer file: `PKM/My Life/Offers/a-year-of-emails.md`, live since 2021, its own sales page [[Library/Examples/sales-pages/a-year-of-emails-sales-page]], and years of Journal ideation). YOE's **sales page and offer file exist**, but the *course/program content* (module structure, teaching material) that `programs/yoe/` was meant to hold was never imported — same as CTC, it sits in [[Studio/Analysis/BUILD-PLAN]] as "YOE content index — not started" and "YOE email concepts → not started" (a Notion vault waiting on Mack's API connection, per BUILD-PLAN Phase 3).
...
- [[Library/Programs/back-of-house/webinars/Out of Office Webinar]]
...
- [[Studio/Team Knowledge/session-logs/2026/06/2026-06-04_larry_second-brain-transcript-mass-import]] — filed [[Library/Programs/back-of-house/webinars/Slow Marketing Plan Class]] into `programs/back-of-house/webinars/` in the same session that did the 21-transcript BOH mass import.
- [[Studio/Team Knowledge/session-logs/2026/06/2026-06-04_pax_webinar-corpus-analysis-methodology]] — Pax's methodology note for classifying "teaching webinar" vs. "pitch-class," written while building [[Studio/Analysis/workshop-webinar-analysis]] from these same three files.
- [[Studio/Analysis/workshop-webinar-analysis]] — the finished cross-webinar synthesis, sourcing all three files explicitly by path.
- [[Studio/Analysis/BUILD-PLAN]] — "Webinars — [[Library/Programs/back-of-house/webinars/Slowly Viral Class]] + [[Library/Programs/back-of-house/webinars/Out of Office Webinar]] + [[Library/Programs/back-of-house/webinars/Slow Marketing Plan Class]] ✅"

**Conclusion:** the empty `webinar-1/`/`webinar-2/` folders are dead scaffold names from before the BOH import happened. Once the three real webinars arrived, the team filed them under the program they actually belong to (BOH) instead of retiring or renaming the generic placeholder slots. Nobody went back to delete `webinar-1/`, `webinar-2/`, `ctc/`, or `yoe/` from `programs/INDEX.md`, so they still read as open/promised work. This exact finding was independently reached by two other passes already in the vault ([[Deliverables/2026-07-05-vault-learn/learn-programs]] and [[Hermes/MAP]] line 89: *"programs/ctc, yoe, webinar-1, webinar-2 = empty placeholders; transcripts never imported"*) — cross-confirmed, not a one-off read.
...
- [[Studio/Analysis/carousel]] — 9 top carousel post-mortems, performance data (✅ 2026-06-03)
- [[Studio/Analysis/carousel-slide-by-slide]] — full slide-by-slide breakdown, imported from Claude OS source (✅ 2026-07-04)
- [[Studio/Analysis/carousel-flow-and-weird-hooks]] — exists, **not listed in `analysis/INDEX.md`**
- [[Studio/Analysis/carousel-hook-forensics]] — exists, **not listed in `analysis/INDEX.md`**

There is **no file called `carousel-analysis.md`** and never has been — the INDEX and BUILD-PLAN have only ever pointed to [[Studio/Analysis/carousel]] and [[Studio/Analysis/carousel-slide-by-slide]].

### Live files still linking the wrong name `[[PKM/Second Brain/analysis/carousel-analysis.md]]`

- [[Studio/Signals/Developing Ideas/email-to-content-repurposing-system]] (two references, lines 10 and 34)
...
- [[Studio/Analysis/boh-teaching-index-phases-0-2]]
- [[Studio/Analysis/boh-teaching-index-phases-3-5]]
- [[Studio/Analysis/carousel-flow-and-weird-hooks]]
- [[Studio/Analysis/carousel-hook-forensics]]
- [[Studio/Analysis/email-microstyle-forensics]]
- [[Studio/Analysis/simmer-opening-forensics]]
...
### [[PKM/Second Brain/INDEX]] (master) and `programs/INDEX.md`
...
**Real gap: Hermes is missing.** `Team/Hermes - Alyssa Clone Writer/` exists as a folder, `.claude/agents/hermes.md` exists as a bound subagent shim, and per the user's own memory index Hermes is "an active AI team member... v2 rebuilt 2026-07-05" — but [[Studio/Team/agent-index]]'s routing table has no Hermes row. Anyone routing purely off [[Studio/Team/agent-index]] (as Larry is instructed to do "on every request") would never discover Hermes exists. This is consistent with Hermes being hired/rebuilt in a session that updated the Team folder and the `.claude/agents/` shim but never touched the routing table itself.

Secondary note: [[Studio/Team/agent-index]] still lists Wren as fully active for all copy tasks, while the user's memory notes "Wren deprecated for voice" as of the Hermes v2 rebuild. The index has not caught up with that decision either — same failure mode as the Hermes gap, just the inverse (stale entry vs. missing entry).
...
- 7 Journal-dated wikilinks from [[Studio/Signals/Developing Ideas/email-to-content-repurposing-system]] (all 7 resolve to real files under `PKM/Journal/`)
- `[[business]]`, `[[emails]]`, `[[offers]]`, `[[promos]]`, `[[socials]]` (from the same file, and from [[Notebook/Life/Key Elements/Business/emails]], [[Notebook/Life/Key Elements/Business/promos]]) — all resolve to real Key Element files
- 5 Developing Ideas garden notes referenced from [[Studio/Signals/signals-index]] (`middle-ground-entrepreneur`, `email-to-content-repurposing-system`, `ai-team-as-mirror-not-replacement`, `boh-as-permission-not-product`, `slow-biz-creative-freedom`) — all resolve
- [[Notebook/Journal/2026/06/2026-06-27-sustainable-creative-biz-signal]] (linked from Signals) — resolves
