...
## Stage 2.5 — The sequence fill (from [[Studio/Content/carousel-dispatch]] Step 1; ONE artifact, simplified 2026-07-17 on her "too much up front work" ruling. SKIPPED in her-piece mode: when Alyssa supplies a finished piece of her writing, dispatch Step 0 applies — no card, no fill, her piece is the plan, one version)
...
4. **Spine + framework execution:** confirm all SEVEN spine jobs are present (cover hook, second hook, old way/problem, proof or validation, teaching, turn, CTA) AND each framework step appears. The usual miss is spine job 4 (proof/validation), which How To's sequence drops. A missing spine job or framework step is a fail, not a nitpick. In her-piece mode (dispatch Step 0) there is no framework card; check the spine jobs against her piece's own order instead.
4b. **Want-before-tool + one-slide payoff (her ruling 2026-07-19):** if the deck gives away a stealable asset (template, checklist, prompt list), that asset must appear WHOLE on exactly ONE slide, and every slide before it must be persuading (why regular, why this kind, what it costs to not have it), not teaching the asset's parts. An asset split across teaching slides is a fail, not a nitpick. The payoff slide is exempt from the word cap.
...
6. **The three gates** from METHOD section 7: lineup test against two real posts, stranger test (name the bit), value test (name what a non-buyer walks away with). The lineup's two real posts are read from [[Studio/Content/nine-decks]] (verbatim slide copy, nine decks plus Mar 1) and, for a For The Girls deck, [[Library/Examples/Carousels/for-the-girls-slide-copy]]. The dated folders under Library/Examples/Carousels are slide images, not copy; never name one as a lineup source.
