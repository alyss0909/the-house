...
**Control Room sync (added 2026-07-16, Alyssa-approved).** Every close-session also checks: did this session create, change, pause, or retire any slash command, skill, SOP, workstream, guideline, engine, routine, or agent? If yes, update the matching rows in the House Operations Board in Notion (inside the "House Control Room" page, data source `collection://fc40825c-e3fc-401a-aa72-73b285604f4c`), setting "Last checked" to today. That board is Alyssa's single window into everything running behind the scenes; keeping it current is part of the close, not optional. The same page holds the Agent Tasks database (data source `collection://c435003e-92d4-4f61-9696-1b86feb46556`), a mirror of `Studio/Team Knowledge/tasks/open/`: new open tasks get a row there (Room and Desk tags, and an "Open in vault" link in the form `https://alyss0909.github.io/vault-links/?f=<URL-encoded path without .md>` — never a raw obsidian:// URI, Notion cannot open those; see `scripts/obsidian-redirect/README.md`), closed tasks get marked done. The sync is two-way (contract expanded 2026-07-16, Alyssa's ask): her "Notes for the team" text on any row is a ruling to honor (a "don't care anymore" note cancels the task; other notes get quoted into the vault task file and obeyed — after acting, append " [seen YYYY-MM-DD]" to the note, never edit her words); a row she sets to Status done gets its vault file closed with no questions; a "Do it" checked box queues the task for the twice-daily sweep or the next live session; and a row with an empty "Open in vault" link is a NEW task she created in Notion — create the vault file, backfill the link, default the assignee to larry. The vault files stay the working source of truth; Notion is the surface Alyssa acts on.
