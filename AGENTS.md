...
## The compounding loop (MANDATORY, added 2026-07-11 — Alyssa's ruling: "every agent, folder, session and action learns from the last")

**Boot:** immediately after [[HOUSE-MAP]], read [[NOW]] at the vault root — the resume surface, regenerated at every close. It carries the last session's hand-off, open threads, due check-backs, the journal resurface duty, and the debt counters. A session that skips NOW.md is operating blind and in violation of this contract.

**The journal is the core (Alyssa, 2026-07-11: "everything in the journal MUST be referenced and brought forward... it should be self referential it should be connecting more and more each day").** Every session performs the journal duty listed in NOW.md: read the resurface candidates, wikilink each into what it genuinely relates to (or cite it from the map/garden note it should feed), and connect any new entries. Work that draws on a journal entry cites it. Every session leaves the journal more connected than it found it. Notebook rules unchanged: these are connective edits only — never her words, never her meaning (GL-013 Rule 1).

**The trigger word (added 2026-07-11): "compound".** When Alyssa says it, in any sentence, the acting agent re-reads [[GL-014-the-compounding-architecture]], audits the current work against its five laws in one short block, and fixes any leak in the same session. One word from her is the entire instruction.

**The team brain (added 2026-07-11, Alyssa's ruling: agents learn from one another and understand her deeper each session).** Shared layer: [[Studio/Team/TEAM-BRAIN]] — her cross-cutting preferences, verbatim rulings, and the precedent index. Per-desk layer: each specialist's `PLAYBOOK.md` (model: Suki's). Larry's Brief step MUST bundle the specialist's playbook, the relevant TEAM-BRAIN slice, and the nearest precedent — no specialist starts cold. Specialists self-gate against their Pre-reject list before Alyssa sees anything. Journal boundary (LOCKED, her words 2026-07-11: "my journal is not written over... that stays Alyssa only"): connections are made FROM Studio files; in-entry edits only on her explicit per-case request.

**Close:** no session ends without [[SOP-040-the-compounding-loop]]: (1) a fate wikilink or dated expiry on every Decision/Insight/Realignment line; (2) the open-thread diff balances — a thread leaves the set only with a `died because:` line; (3) any insight appearing for the second time anywhere becomes a drafted proposal flagged needs-Alyssa in the same session; (4) journal duty done; (5) rewrite the hand-off block in NOW.md and run `python3 scripts/regenerate-now.py` (manual fallback in SOP-040) so the next session boots from compounded state; (6) commit and `git push origin master` — no session closes until its work is backed up to GitHub (SOP-040 step 9, Alyssa-approved 2026-07-17).
...
**The house check (added 2026-07-20, Alyssa-approved).** Before writing the hand-off, run `python3 scripts/house-check.py` and clear every line it prints. It is not advisory. A **LEAK** means an open thread vanished between session logs without becoming a task or being resolved; it gets a task file or an explicit line saying why it died. **UNCAPTURED** means NOW.md carries an item with no task behind it; same rule. **DRIFT** means a task file's status disagrees with its folder; run `python3 scripts/rebuild-task-index.py`. **STALE** means an open task is 14 days or older; say something about it in the hand-off.

This check exists because on 2026-07-20 an audit found roughly 75 pieces of committed work that had never become tasks, and a Notion board that had drifted from the vault for days. The cause was that every guardrail here was a rule an agent was asked to REMEMBER rather than a check that RUNS. Rules like that fail silently and the failure compounds. The check runs at session boot via the SessionStart hook and again at close. Do not treat a clean run as permission to skip the sync below.

**Control Room sync (added 2026-07-16, Alyssa-approved; made unconditional 2026-07-20).** Every close-session reconciles Notion against the vault, every time, with no "nothing changed" exemption — that exemption is precisely how the board drifted. Run `python3 scripts/house-check.py --json` for the canonical open-task list, query the Agent Tasks database, and diff the two: a task file in `tasks/open/` with no row gets a row, a row not open in the vault gets its Status corrected, and killed work is marked done with a "Notes for the team" line saying it was killed and why, so the board never implies something was finished when it was abandoned. Then update the matching rows in the House Operations Board in Notion (inside the "House Control Room" page, data source `collection://fc40825c-e3fc-401a-aa72-73b285604f4c`), setting "Last checked" to today. That board is Alyssa's single window into everything running behind the scenes; keeping it current is part of the close, not optional. The same page holds the Agent Tasks database (data source `collection://c435003e-92d4-4f61-9696-1b86feb46556`), a mirror of `Studio/Team Knowledge/tasks/open/`: new open tasks get a row there (Room and Desk tags, and an "Open in vault" link in the form `https://alyss0909.github.io/vault-links/?f=<URL-encoded path without .md>` — never a raw obsidian:// URI, Notion cannot open those; see `scripts/obsidian-redirect/README.md`), closed tasks get marked done. The sync is two-way (contract expanded 2026-07-16, Alyssa's ask): her "Notes for the team" text on any row is a ruling to honor (a "don't care anymore" note cancels the task; other notes get quoted into the vault task file and obeyed — after acting, append " [seen YYYY-MM-DD]" to the note, never edit her words); a row she sets to Status done gets its vault file closed with no questions; a "Do it" checked box queues the task for the twice-daily sweep or the next live session; and a row with an empty "Open in vault" link is a NEW task she created in Notion — create the vault file, backfill the link, default the assignee to larry. The vault files stay the working source of truth; Notion is the surface Alyssa acts on.
