...
Before writing or answering in Alyssa's voice, load [[Voice/README]] (this file), then the relevant file in [[Voice/formats]] for the format you're writing. Write toward her real examples first. Run [[Voice/KILL-LIST]] as the last pass, after a draft exists, not before.

## Authority order

1. **Her real published examples come first.** [[Library/Examples]] holds the primary source: real sales emails, Simmers, Soft Sundays, sales pages, carousels, and website copy she actually shipped. You write toward these. See the format-by-format breakdown in [[Voice/formats/README]].
2. **The kill list is the editing pass, run after a draft exists.** [[Voice/KILL-LIST]] is not a writing guide. It is a checklist you run against a finished draft to catch AI tells: banned reframe shapes, banned vocabulary, punctuation rules, and the rule-priority order. It is sourced from [[Hermes/ANTI-AI]], which is itself built from Alyssa's own journal words, not from AI inference about her.
...
- **Whether [[AGENTS.md]] gets a mandatory-load rule for [[Voice/README]]**, so every specialist is required to load this layer before writing or answering in Alyssa's voice, not just the ones who happen to remember to check.
