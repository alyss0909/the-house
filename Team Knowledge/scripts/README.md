...
**Status:** optional repair utility. Safe to keep.

### What it does

Repairs the common Windows encoding failure where UTF-8 text was accidentally
read as Windows-1252 and then saved, producing broken punctuation, arrows,
checkmarks, or emoji. For example, the broken codepoint run
`\u00e2\u20ac\u201d` repairs back to an em dash.
...
After any batch import, transcript filing, or generated-file cleanup pass, run
the dry-run command before calling the work complete. If it reports changed
files, apply the repair and scan again.
